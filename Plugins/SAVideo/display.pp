...
const _msusrmax=$10000;
const _mscntmax=$10000;
...
var _wc:wndClass;                 //窗口注册结构
    _ms:msg;                      //消息结构
    _msusr:array[1.._msusrmax]of msg;//用户消息结构
    _msusri:longword=1;
    _msi:longword=1;
    _mscnt:array[0.._mscntmax-1]of real;//消息时间计数
    _mscnti:longword=0;
    _mscntj:longword=0;
    _main:bitmap;                 //缓冲位图结构
...
procedure DrawTextXY(b:pbitmap;s:ansistring;x,y:longword;c:longword);
procedure DrawTextXY(s:ansistring;x,y:longword;cfg,cbg:longword);
procedure DrawTextXY(s:ansistring;x,y:longword;c:longword);
...
procedure Bar(x,y:longword;w,h:longint;cfg,cbg:longword);
...
_x,_y,_w,_h,0,0,MainInstance,nil);
end;
...
begin IsNextMsg:=PeekMessage(_ms,_hw,0,0,PM_REMOVE);if IsNextMsg then SendMsg();end
else IsNextMsg:=true;
end;
function GetNextMsg():longword;
...
begin
if not(CustomMsg()) then
begin GetMessage(_ms,_hw,0,0);SendMsg();end;
